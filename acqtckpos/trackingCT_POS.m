...
%                                                 + (1/cmn.cSpeed)*sv_clk(prn);
        ...
        elseif solu.mode==1
%             RAIM positioning
            prvec   = pseudorange - sv_clk(prn) - grpdel*cmn.cSpeed;
            cn0 = CN0_CT(posIndex, :);
%             cn0 = Acquired.SNR
            current_svxyzr = squeeze(svxyzr(posIndex, :, :)); % extract 2D 
            [estusr_wls, dop, fault_sv] = raim_wls(prvec, current_svxyzr, cn0, estusr_wls, 1e-3, 10);
        ...
        elseif solu.mode==2         
            % EKF positioning   
            doppler_mps = (carrFreq - signal.IF) * cmn.cSpeed / signal.Fc- sv_clk_vel(svindex);  % Units：m/s
...
%         navSolutionsWLS.DOP(posIndex,:)       = dop;
        ...
        navSolutionsWLS.svxyz(posIndex, :) = svxyz;   % 三维：时间 x 卫星数 x 坐标 % 保存当前时刻所有卫星的ECEF位置和速度
        navSolutionsWLS.sv_vel(posIndex, :) = sv_vel;
...
elseif solu.mode==1
    %Save RAIM result
    raim_filename = ['navSolCT_RAIM_', num2str(track.pdi), 'ms_', file.fileName, '.mat'];
    save(raim_filename, 'navSolutionsCT');
    save(['tckRstCT_RAIM',num2str(pdi),'ms_',file.fileName], 'TckResultCT_POS','CN0_CT');
    fprintf('RAIM result saves to : %s\n', raim_filename);
    
elseif solu.mode == 2
    % save EKF result
    ekf_filename = ['navSolCT_EKF_', num2str(track.pdi), 'ms_', file.fileName, '.mat'];
    save(ekf_filename, 'navSolutionsCT');
    save(['tckRstCT_EKF',ekf_fg2str(pdi),'ms_',file.fileName], 'TckResultCT_POS','CN0_CT');
