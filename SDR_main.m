...
if cmn.vtEnable == 1    
    ...
    %----------------------------------------------------------------------
    % RAIM positioning mode£¨solu.mode=1£©
    if solu.mode == 1
        Raim_filename = ['navSolCT_RAIM_', num2str(track.pdi), 'ms_', file.fileName, '.mat'];
        if exist(Raim_filename, 'file')
            % if file exits,directly save the results
            fprintf('RAIM result exist, skipping calculation: %s\n', Raim_filename);
            load(Raim_filename, 'navSolutionsCT');
        else
            % File doesn't exist,perform RAIM positioning and save the results
            fprintf('Running RAIM Positioning...\n');
            [TckResultCT_POS, navSolutionsCT] = trackingCT_POS(...
                file,signal,track,cmn, Acquired,TckResult_Eph,cnslxyz,eph,sbf,solu...
                );
    ...
    % ---------------------------------------------------------------------
    % EKF positioning mode£¨solu.mode=2£©
    if solu.mode == 2 
...
plotDir = 'D:/00_HK/Class_GNSS/AAE_Assign2/GNSSSDR_LWT/'
